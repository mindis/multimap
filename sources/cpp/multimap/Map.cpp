...
std::size_t Map::ReplaceAll(const Bytes& key, const Bytes& old_value,
                            const Bytes& new_value) {
  return UpdateAll(key, [&old_value, &new_value](const Bytes& current_value) {
    return (current_value == old_value) ? new_value.ToString() : std::string();
  });
}
...
void Map::Copy(const boost::filesystem::path& from,
  ...
  Copy(from, to, Compare(), new_block_size);
}
