...
void jenkins_state_destroy(jenkins_state_t *state) { free(state); }

static inline void __jenkins_hash_vector(cmph_uint32 seed, const char *k,
                                         cmph_uint32 keylen,
                                         cmph_uint32 *hashes) {
  ...
  while (len >= 12) {
    hashes[0] += ((cmph_uint32)k[0] + ((cmph_uint32)k[1] << 8) +
                  ((cmph_uint32)k[2] << 16) + ((cmph_uint32)k[3] << 24));
    hashes[1] += ((cmph_uint32)k[4] + ((cmph_uint32)k[5] << 8) +
                  ((cmph_uint32)k[6] << 16) + ((cmph_uint32)k[7] << 24));
    hashes[2] += ((cmph_uint32)k[8] + ((cmph_uint32)k[9] << 8) +
                  ((cmph_uint32)k[10] << 16) + ((cmph_uint32)k[11] << 24));
...
cmph_uint32 jenkins_hash(jenkins_state_t *state, const char *k,
                         cmph_uint32 keylen) {
  ...
  /*	cmph_uint32 a, b, c;
          ...
          while (len >= 12)
          {
                  a += (k[0] +((cmph_uint32)k[1]<<8) +((cmph_uint32)k[2]<<16)
     +((cmph_uint32)k[3]<<24));
                  b += (k[4] +((cmph_uint32)k[5]<<8) +((cmph_uint32)k[6]<<16)
     +((cmph_uint32)k[7]<<24));
                  c += (k[8] +((cmph_uint32)k[9]<<8)
     +((cmph_uint32)k[10]<<16)+((cmph_uint32)k[11]<<24));
                  mix(a,b,c);
                  k += 12; len -= 12;
          }
...
void jenkins_hash_vector_(jenkins_state_t *state, const char *k,
                          cmph_uint32 keylen, cmph_uint32 *hashes) {
  __jenkins_hash_vector(state->seed, k, keylen, hashes);
}

void jenkins_state_dump(jenkins_state_t *state, char **buf,
                        cmph_uint32 *buflen) {
...
jenkins_state_t *jenkins_state_copy(jenkins_state_t *src_state) {
  jenkins_state_t *dest_state =
      (jenkins_state_t *)malloc(sizeof(jenkins_state_t));
 ...
 *  \brief Support the ability to pack a jenkins function into a preallocated
 * contiguous memory space pointed by jenkins_packed.
 *  \param state points to the jenkins function
 *  \param jenkins_packed pointer to the contiguous memory area used to store
 * the jenkins function. The size of jenkins_packed must be at least
 * jenkins_state_packed_size()
 */
 ...
 */
cmph_uint32 jenkins_state_packed_size(void) { return sizeof(cmph_uint32); }

/** \fn cmph_uint32 jenkins_hash_packed(void *jenkins_packed, const char *k,
 * cmph_uint32 keylen);
 ...
 */
cmph_uint32 jenkins_hash_packed(void *jenkins_packed, const char *k,
                                cmph_uint32 keylen) {
  cmph_uint32 hashes[3];
...
/** \fn jenkins_hash_vector_packed(void *jenkins_packed, const char *k,
 * cmph_uint32 keylen, cmph_uint32 * hashes);
 *  \param jenkins_packed is a pointer to a contiguous memory area
 *  \param key is a pointer to a key
 *  \param keylen is the key length
 *  \param hashes is a pointer to a memory large enough to fit three 32-bit
 * integers.
 */
void jenkins_hash_vector_packed(void *jenkins_packed, const char *k,
                                cmph_uint32 keylen, cmph_uint32 *hashes) {
  __jenkins_hash_vector(*((cmph_uint32 *)jenkins_packed), k, keylen, hashes);
}
