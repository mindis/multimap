...
const cmph_uint8 bdz_lookup_table[] = {
    4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2, 4, 4, 4, 3, 4, 4, 4, 3,
    4, 4, 4, 3, 3, 3, 3, 2, 4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2,
    3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 2, 2, 2, 1, 4, 4, 4, 3, 4, 4, 4, 3,
    4, 4, 4, 3, 3, 3, 3, 2, 4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2,
    4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2,
    3, 3, 3, 2, 2, 2, 2, 1, 4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2,
    4, 4, 4, 3, 4, 4, 4, 3, 4, 4, 4, 3, 3, 3, 3, 2, 4, 4, 4, 3, 4, 4, 4, 3,
    4, 4, 4, 3, 3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 2, 2, 2, 1,
    3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 2, 2, 2, 1, 3, 3, 3, 2, 3, 3, 3, 2,
    3, 3, 3, 2, 2, 2, 2, 1, 3, 3, 3, 2, 3, 3, 3, 2, 3, 3, 3, 2, 2, 2, 2, 1,
...
static void bdz_alloc_graph3(bdz_graph3_t *graph3, cmph_uint32 nedges,
...
static void bdz_init_graph3(bdz_graph3_t *graph3,
                            cmph_uint32 nedges __attribute__((unused)),
                            cmph_uint32 nvertices) {
...
static void bdz_add_edge(bdz_graph3_t *graph3, cmph_uint32 v0, cmph_uint32 v1,
  ...
  graph3->first_edge[v0] = graph3->first_edge[v1] = graph3->first_edge[v2] =
      graph3->nedges;
...
static void bdz_dump_graph(bdz_graph3_t *graph3, cmph_uint32 nedges,
                           cmph_uint32 nvertices) {
  cmph_uint32 i;
  for (i = 0; i < nedges; i++) {
...
static void bdz_remove_edge(bdz_graph3_t *graph3, cmph_uint32 curr_edge) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (edge1 == NULL_EDGE) {
      ...
      bdz_dump_graph(graph3, graph3->nedges,
                     graph3->nedges + graph3->nedges / 4);
      exit(-1);
    };
...
static int bdz_generate_queue(cmph_uint32 nedges,
                              cmph_uint32 nvertices __attribute__((unused)),
                              bdz_queue_t queue, bdz_graph3_t *graph3) {
  ...
  for (i = 0; i < nedges; i++) {
    ...
    if (graph3->vert_degree[v0] == 1 || graph3->vert_degree[v1] == 1 ||
  ...
  return (int)(queue_head -
               nedges); /* returns 0 if successful otherwies return negative
                           number*/
};

static int bdz_mapping(cmph_config_t *mph, bdz_graph3_t *graph3,
                       bdz_queue_t queue);
static void assigning(bdz_config_data_t *bdz, bdz_graph3_t *graph3,
                      bdz_queue_t queue);
static void ranking(bdz_config_data_t *bdz);
static cmph_uint32 rank(cmph_uint32 b, cmph_uint32 *ranktable, cmph_uint8 *g,
                        cmph_uint32 vertex);
...
cmph_t *bdz_new(cmph_config_t *mph, double c) {
  ...
  if (mph->verbosity) {
    fprintf(stderr,
            "Entering mapping step for mph creation of %u keys with graph "
            "sized %u\n",
            bdz->m, bdz->n);
  }
  while (1) {
    ...
    if (!ok) {
      ...
      if (mph->verbosity) {
        fprintf(stderr,
                "acyclic graph creation failure - %u iterations remaining\n",
                iterations);
      }
  ...
  if (mph->verbosity) {
    fprintf(stderr,
            "Entering assigning step for mph creation of %u keys with graph "
            "sized %u\n",
            bdz->m, bdz->n);
  }
  ...
  if (mph->verbosity) {
    fprintf(stderr,
            "Entering ranking step for mph creation of %u keys with graph "
            "sized %u\n",
            bdz->m, bdz->n);
  }
  ranking(bdz);
...
static void assigning(bdz_config_data_t *bdz, bdz_graph3_t *graph3,
  ...
  for (i = nedges - 1; i + 1 >= 1; i--) {
    ...
    v1 = graph3->edges[curr_edge].vertices[1];
    v2 = graph3->edges[curr_edge].vertices[2];
    DEBUGP("B:%u %u %u -- %u %u %u\n", v0, v1, v2, GETVALUE(bdz->g, v0),
           GETVALUE(bdz->g, v1), GETVALUE(bdz->g, v2));
    if (!GETBIT(marked_vertices, v0)) {
      ...
      SETVALUE1(bdz->g, v0,
                (6 - (GETVALUE(bdz->g, v1) + GETVALUE(bdz->g, v2))) % 3);
      ...
      SETVALUE1(bdz->g, v1,
                (7 - (GETVALUE(bdz->g, v0) + GETVALUE(bdz->g, v2))) % 3);
      SETBIT(marked_vertices, v1);
    } else {
      SETVALUE1(bdz->g, v2,
                (8 - (GETVALUE(bdz->g, v0) + GETVALUE(bdz->g, v1))) % 3);
      SETBIT(marked_vertices, v2);
    }
...
static void ranking(bdz_config_data_t *bdz) {
  cmph_uint32 i, j, offset = 0U, count = 0U, size = (bdz->k >> 2U),
                    nbytes_total = (cmph_uint32)ceil(bdz->n / 4.0), nbytes;
  bdz->ranktable =
      (cmph_uint32 *)calloc((size_t)bdz->ranktablesize, sizeof(cmph_uint32));
...
int bdz_dump(cmph_t *mphf, FILE *fd) {
  ...
  nbytes = fwrite(data->ranktable, sizeof(cmph_uint32) * (data->ranktablesize),
                  (size_t)1, fd);
#ifdef DEBUG
  cmph_uint32 i;
  fprintf(stderr, "G: ");
...
void bdz_load(FILE *f, cmph_t *mphf) {
  ...
  bdz->ranktable =
      (cmph_uint32 *)calloc((size_t)bdz->ranktablesize, sizeof(cmph_uint32));
  nbytes = fread(bdz->ranktable, sizeof(cmph_uint32) * (bdz->ranktablesize),
                 (size_t)1, f);
...
static inline cmph_uint32 rank(cmph_uint32 b, cmph_uint32 *ranktable,
                               cmph_uint8 *g, cmph_uint32 vertex) {
...
cmph_uint32 bdz_search(cmph_t *mphf, const char *key, cmph_uint32 keylen) {
  ...
  vertex = hl[(GETVALUE(bdz->g, hl[0]) + GETVALUE(bdz->g, hl[1]) +
               GETVALUE(bdz->g, hl[2])) %
              3];
 ...
 *  \brief Support the ability to pack a perfect hash function into a
 * preallocated contiguous memory space pointed by packed_mphf.
 *  \param mphf pointer to the resulting mphf
 *  \param packed_mphf pointer to the contiguous memory area used to store the
 * resulting mphf. The size of packed_mphf must be at least cmph_packed_size()
 */
...
cmph_uint32 bdz_packed_size(cmph_t *mphf) {
  ...
  return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_packed_size(hl_type) +
                       ...
                       sizeof(cmph_uint32) * (data->ranktablesize) +
                       sizeof(cmph_uint8) +
                       sizeof(cmph_uint8) * (cmph_uint32)(ceil(data->n / 4.0)));
}

/** cmph_uint32 bdz_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 bdz_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
  register cmph_uint32 vertex;
  ...
  hl[2] = hl[2] % r + (r << 1);
  vertex =
      hl[(GETVALUE(g, hl[0]) + GETVALUE(g, hl[1]) + GETVALUE(g, hl[2])) % 3];
