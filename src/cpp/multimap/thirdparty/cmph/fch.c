...
static cmph_uint8 check_for_collisions_h2(fch_config_data_t *fch,
                                          fch_buckets_t *buckets,
                                          cmph_uint32 *sorted_indexes);
static void permut(cmph_uint32 *vector, cmph_uint32 n);
static cmph_uint8 searching(fch_config_data_t *fch, fch_buckets_t *buckets,
                            cmph_uint32 *sorted_indexes);
...
cmph_uint32 mixh10h11h12(cmph_uint32 b, double p1, double p2,
                         cmph_uint32 initial_index) {
  register cmph_uint32 int_p2 = (cmph_uint32)p2;
  if (initial_index < p1)
    initial_index %= int_p2; /* h11 o h10 */
  else {                     /* h12 o h10 */
    initial_index %= b;
...
double fch_calc_p1(cmph_uint32 m) { return ceil(0.55 * m); }

double fch_calc_p2(cmph_uint32 b) { return ceil(0.3 * b); }
...
static cmph_uint8 check_for_collisions_h2(fch_config_data_t *fch,
  ...
  cmph_uint8 *hashtable =
      (cmph_uint8 *)calloc((size_t)fch->m, sizeof(cmph_uint8));
  ...
  for (i = 0; i < nbuckets; i++) {
    ...
    for (j = 0; j < nkeys; j++) {
      ...
      cmph_uint32 keylen =
          fch_buckets_get_keylength(buckets, sorted_indexes[i], j);
...
static cmph_uint8 searching(fch_config_data_t *fch, fch_buckets_t *buckets,
  ...
  cmph_uint32 *random_table =
      (cmph_uint32 *)calloc((size_t)fch->m, sizeof(cmph_uint32));
  ...
  do {
    ...
    for (i = 0; (i < nbuckets) && !restart; i++) {
      ...
      for (z = 0; (z < (fch->m - filled_count)) && restart; z++) {
        ...
        cmph_uint32 keylen =
            fch_buckets_get_keylength(buckets, sorted_indexes[i], INDEX);
        cmph_uint32 h2 = hash(fch->h2, key, keylen) % fch->m;
        counter = 0;
        restart = 0;  // false
        fch->g[sorted_indexes[i]] =
            (fch->m + random_table[filled_count + z] - h2) % fch->m;
...
cmph_uint32 fch_packed_size(cmph_t *mphf) {
  ...
  return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_packed_size(h1_type) +
                       ...
                       4 * sizeof(cmph_uint32) + 2 * sizeof(double) +
                       sizeof(cmph_uint32) * (data->b));
}

/** cmph_uint32 fch_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 fch_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
