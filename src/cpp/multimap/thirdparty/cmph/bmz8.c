...
static cmph_uint8 bmz8_traverse_critical_nodes(bmz8_config_data_t *bmz8,
                                               cmph_uint32 v,
                                               cmph_uint8 *biggest_g_value,
                                               cmph_uint8 *biggest_edge_value,
                                               cmph_uint8 *used_edges,
                                               cmph_uint8 *visited);
static cmph_uint8 bmz8_traverse_critical_nodes_heuristic(
    bmz8_config_data_t *bmz8, cmph_uint32 v, cmph_uint8 *biggest_g_value,
    cmph_uint8 *biggest_edge_value, cmph_uint8 *used_edges,
    cmph_uint8 *visited);
static void bmz8_traverse_non_critical_nodes(bmz8_config_data_t *bmz8,
                                             cmph_uint8 *used_edges,
                                             cmph_uint8 *visited);
  ...
  do {
    ...
    while (1) {
      ...
      if (!ok) {
        ...
        if (mph->verbosity) {
          fprintf(stderr,
                  "simple graph creation failure - %u iterations remaining\n",
                  iterations);
        }
    ...
    for (i = 0; i < bmz8->n; ++i)  // critical nodes
    {
      if (graph_node_is_critical(bmz8->graph, i) && (!GETBIT(visited, i))) {
        if (c > 1.14)
          restart_mapping = bmz8_traverse_critical_nodes(
              bmz8, i, &biggest_g_value, &biggest_edge_value, used_edges,
              visited);
        else
          restart_mapping = bmz8_traverse_critical_nodes_heuristic(
              bmz8, i, &biggest_g_value, &biggest_edge_value, used_edges,
              visited);
    ...
    if (!restart_mapping) {
      ...
      bmz8_traverse_non_critical_nodes(bmz8, used_edges,
                                       visited);  // non_critical_nodes
    } else {
      iterations_map--;
      if (mph->verbosity)
...
static cmph_uint8 bmz8_traverse_critical_nodes(bmz8_config_data_t *bmz8,
  ...
  next_g = (cmph_uint8)floor(
      (double)(*biggest_edge_value /
               2)); /* next_g is incremented in the do..while statement*/
  vqueue_insert(q, v);
    ...
    while ((u = graph_next_neighbor(bmz8->graph, &it)) != GRAPH_NO_NEIGHBOR) {
      if (graph_node_is_critical(bmz8->graph, u) && (!GETBIT(visited, u))) {
        ...
        while (collision)  // lookahead to resolve collisions
        {
          ...
          collision = 0;
          while ((lav = graph_next_neighbor(bmz8->graph, &it1)) !=
                 GRAPH_NO_NEIGHBOR) {
            if (graph_node_is_critical(bmz8->graph, lav) &&
                GETBIT(visited, lav)) {
    ...
    while ((u = graph_next_neighbor(bmz8->graph, &it)) != GRAPH_NO_NEIGHBOR) {
      if (graph_node_is_critical(bmz8->graph, u) && (!GETBIT(visited, u))) {
        ...
        while (collision)  // lookahead to resolve collisions
        {
          ...
          if (collision && (next_g > *biggest_g_value))  // saving the current g
                                                         // value stored in
                                                         // next_g.
          {
            if (nunused_g_values == unused_g_values_capacity) {
              unused_g_values = (cmph_uint8 *)realloc(
                  unused_g_values,
                  ((size_t)(unused_g_values_capacity + BUFSIZ)) *
                      sizeof(cmph_uint8));
...
static cmph_uint8 next_unused_edge(bmz8_config_data_t *bmz8,
                                   cmph_uint8 *used_edges,
...
static void bmz8_traverse(bmz8_config_data_t *bmz8, cmph_uint8 *used_edges,
                          cmph_uint32 v, cmph_uint8 *unused_edge_index,
                          cmph_uint8 *visited) {
  graph_iterator_t it = graph_neighbors_it(bmz8->graph, v);
  cmph_uint32 neighbor = 0;
  while ((neighbor = graph_next_neighbor(bmz8->graph, &it)) !=
         GRAPH_NO_NEIGHBOR) {
...
void bmz8_load(FILE *f, cmph_t *mphf) {
  ...
  bmz8->hashes =
      (hash_state_t **)malloc(sizeof(hash_state_t *) * (size_t)(nhashes + 1));
...
cmph_uint32 bmz8_packed_size(cmph_t *mphf) {
  ...
  return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_packed_size(h1_type) +
                       hash_state_packed_size(h2_type) +
                       2 * sizeof(cmph_uint32) + sizeof(cmph_uint8) +
                       sizeof(cmph_uint8) * data->n);
}

/** cmph_uint8 bmz8_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint8 bmz8_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
  ...
  register cmph_uint8 h1 =
      (cmph_uint8)(hash_packed(h1_ptr, h1_type, key, keylen) % n);
  register cmph_uint8 h2 =
      (cmph_uint8)(hash_packed(h2_ptr, h2_type, key, keylen) % n);
  DEBUGP("key: %s h1: %u h2: %u\n", key, h1, h2);
  if (h1 == h2 && ++h2 > n) h2 = 0;
