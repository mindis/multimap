...
void compressed_rank_generate(compressed_rank_t *cr, cmph_uint32 *vals_table,
  ...
  select_vec =
      (cmph_uint32 *)calloc(cr->max_val >> cr->rem_r, sizeof(cmph_uint32));
  cr->vals_rems = (cmph_uint32 *)calloc(BITS_TABLE_SIZE(cr->n, cr->rem_r),
                                        sizeof(cmph_uint32));
  rems_mask = (1U << cr->rem_r) - 1U;

  for (i = 0; i < cr->n; i++) {
    set_bits_value(cr->vals_rems, i, vals_table[i] & rems_mask, cr->rem_r,
                   rems_mask);
  }
  ...
  // FABIANO: before it was (cr->total_length >> cr->rem_r) + 1. But I wiped out
  // the + 1 because
...
cmph_uint32 compressed_rank_get_space_usage(compressed_rank_t *cr) {
  ...
  space_usage +=
      BITS_TABLE_SIZE(cr->n, cr->rem_r) * (cmph_uint32)sizeof(cmph_uint32) * 8;
...
void compressed_rank_dump(compressed_rank_t *cr, char **buf,
  ...
  register cmph_uint32 vals_rems_size =
      BITS_TABLE_SIZE(cr->n, cr->rem_r) * (cmph_uint32)sizeof(cmph_uint32);
