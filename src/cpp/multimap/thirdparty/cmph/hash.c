...
void hash_vector(hash_state_t *state, const char *key, cmph_uint32 keylen,
                 cmph_uint32 *hashes) {
  switch (state->hashfunc) {
    case CMPH_HASH_JENKINS:
...
void hash_state_dump(hash_state_t *state, char **buf, cmph_uint32 *buflen) {
  ...
  *buf = (char *)malloc(strlen(cmph_hash_names[state->hashfunc]) + 1 + *buflen);
  memcpy(*buf, cmph_hash_names[state->hashfunc],
         strlen(cmph_hash_names[state->hashfunc]) + 1);
  DEBUGP("Algobuf is %u\n", *(cmph_uint32 *)algobuf);
  len = *buflen;
...
hash_state_t *hash_state_copy(hash_state_t *src_state) {
  ...
  switch (src_state->hashfunc) {
    case CMPH_HASH_JENKINS:
      dest_state =
          (hash_state_t *)jenkins_state_copy((jenkins_state_t *)src_state);
      break;
    default:
      assert(0);
  }
 ...
 *  \brief Support the ability to pack a hash function into a preallocated
 *contiguous memory space pointed by hash_packed.
 *  \param state points to the hash function
 *  \param hash_packed pointer to the contiguous memory area used to store the
 *hash function. The size of hash_packed must be at least
 *hash_state_packed_size()
 *
 * Support the ability to pack a hash function into a preallocated contiguous
 *memory space pointed by hash_packed.
...
/** \fn cmph_uint32 hash_packed(void *hash_packed, CMPH_HASH hashfunc, const
 * char *k, cmph_uint32 keylen)
 ...
 */
cmph_uint32 hash_packed(void *hash_packed, CMPH_HASH hashfunc, const char *k,
                        cmph_uint32 keylen) {
  switch (hashfunc) {
    case CMPH_HASH_JENKINS:
...
/** \fn hash_vector_packed(void *hash_packed, CMPH_HASH hashfunc, const char *k,
 ...
 * integers.
 */
void hash_vector_packed(void *hash_packed, CMPH_HASH hashfunc, const char *k,
                        cmph_uint32 keylen, cmph_uint32 *hashes) {
  switch (hashfunc) {
    case CMPH_HASH_JENKINS:
 ...
 */
CMPH_HASH hash_get_type(hash_state_t *state) { return state->hashfunc; }
