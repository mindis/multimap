...
struct __brz_data_t {
  CMPH_ALGO algo;  // CMPH algo for generating the MPHFs for the buckets (Just
                   // CMPH_FCH and CMPH_BMZ8)
  cmph_uint32 m;   // edges (words) count
  double c;        // constant c
  cmph_uint8 *
      size;  // size[i] stores the number of edges represented by g[i][...].
  cmph_uint32 *
      offset;  // offset[i] stores the sum: size[0] + size[1] + ... size[i-1].
