 ...
 *  \brief Support the ability to pack a select structure into a preallocated
 * contiguous memory space pointed by sel_packed.
 ...
 *  \param one_idx is the rank for which we want to calculate the inverse
 * function select
...
/** \fn cmph_uint32 select_next_query_packed(void * sel_packed, cmph_uint32
 * vec_bit_idx);
 *  \param sel_packed is a pointer to a contiguous memory area
 *  \param vec_bit_idx is a value prior computed by @see select_query_packed
 *  \return an integer that represents the next select value greater than @see
 * vec_bit_idx.
 */
