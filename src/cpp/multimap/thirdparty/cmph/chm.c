...
static void chm_traverse(chm_config_data_t *chm, cmph_uint8 *visited,
                         cmph_uint32 v);
...
static int chm_gen_edges(cmph_config_t *mph) {
  ...
  DEBUGP("Generating edges for %u vertices with hash functions %s and %s\n",
         ...
         cmph_hash_names[chm->hashfuncs[1]]);
...
/** cmph_uint32 chm_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 chm_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
