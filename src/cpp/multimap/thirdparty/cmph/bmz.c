...
static cmph_uint8 bmz_traverse_critical_nodes(bmz_config_data_t *bmz,
                                              cmph_uint32 v,
                                              cmph_uint32 *biggest_g_value,
                                              cmph_uint32 *biggest_edge_value,
                                              cmph_uint8 *used_edges,
                                              cmph_uint8 *visited);
static cmph_uint8 bmz_traverse_critical_nodes_heuristic(
    bmz_config_data_t *bmz, cmph_uint32 v, cmph_uint32 *biggest_g_value,
    cmph_uint32 *biggest_edge_value, cmph_uint8 *used_edges,
    cmph_uint8 *visited);
static void bmz_traverse_non_critical_nodes(bmz_config_data_t *bmz,
                                            cmph_uint8 *used_edges,
                                            cmph_uint8 *visited);
  ...
  do {
    ...
    for (i = 0; i < bmz->n; ++i)  // critical nodes
    {
      if (graph_node_is_critical(bmz->graph, i) && (!GETBIT(visited, i))) {
        if (c > 1.14)
          restart_mapping = bmz_traverse_critical_nodes(
              bmz, i, &biggest_g_value, &biggest_edge_value, used_edges,
              visited);
        else
          restart_mapping = bmz_traverse_critical_nodes_heuristic(
              bmz, i, &biggest_g_value, &biggest_edge_value, used_edges,
              visited);
    ...
    if (!restart_mapping) {
      ...
      bmz_traverse_non_critical_nodes(bmz, used_edges,
...
static cmph_uint8 bmz_traverse_critical_nodes(bmz_config_data_t *bmz,
  ...
  vqueue_t *q =
      vqueue_new((cmph_uint32)(graph_ncritical_nodes(bmz->graph)) + 1);
    ...
    while ((u = graph_next_neighbor(bmz->graph, &it)) != GRAPH_NO_NEIGHBOR) {
      if (graph_node_is_critical(bmz->graph, u) && (!GETBIT(visited, u))) {
        ...
        while (collision)  // lookahead to resolve collisions
        {
          ...
          collision = 0;
          while ((lav = graph_next_neighbor(bmz->graph, &it1)) !=
                 GRAPH_NO_NEIGHBOR) {
            if (graph_node_is_critical(bmz->graph, lav) &&
                GETBIT(visited, lav)) {
...
static cmph_uint8 bmz_traverse_critical_nodes_heuristic(
  ...
  vqueue_t *q =
      vqueue_new((cmph_uint32)(0.5 * graph_ncritical_nodes(bmz->graph)) + 1);
  graph_iterator_t it, it1;
                                                         ...
                                                         // next_g.
          {
            if (nunused_g_values == unused_g_values_capacity) {
              unused_g_values = (cmph_uint32 *)realloc(
                  unused_g_values,
                  (unused_g_values_capacity + BUFSIZ) * sizeof(cmph_uint32));
...
static cmph_uint32 next_unused_edge(bmz_config_data_t *bmz,
                                    cmph_uint8 *used_edges,
...
static void bmz_traverse(bmz_config_data_t *bmz, cmph_uint8 *used_edges,
                         cmph_uint32 v, cmph_uint32 *unused_edge_index,
                         cmph_uint8 *visited) {
  graph_iterator_t it = graph_neighbors_it(bmz->graph, v);
  cmph_uint32 neighbor = 0;
  while ((neighbor = graph_next_neighbor(bmz->graph, &it)) !=
         GRAPH_NO_NEIGHBOR) {
...
/** cmph_uint32 bmz_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 bmz_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
  ...
  register cmph_uint32 *g_ptr =
      (cmph_uint32 *)(h2_ptr + hash_state_packed_size(h2_type));
