...
cmph_t *chd_new(cmph_config_t *mph, double c) {
  ...
  if (mph->verbosity) {
    fprintf(stderr,
            "Compressing the range of the resulting CHD_PH perfect hash "
            "function\n");
  }
...
int chd_dump(cmph_t *mphf, FILE *fd) {
  ...
  nbytes =
      fwrite(&data->packed_chd_phf_size, sizeof(cmph_uint32), (size_t)1, fd);
  nbytes =
      fwrite(data->packed_chd_phf, data->packed_chd_phf_size, (size_t)1, fd);
...
static inline cmph_uint32 _chd_search(void *packed_chd_phf, void *packed_cr,
  ...
  register cmph_uint32 bin_idx =
      cmph_search_packed(packed_chd_phf, key, keylen);
