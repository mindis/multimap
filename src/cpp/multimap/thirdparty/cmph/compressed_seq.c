...
void compressed_seq_generate(compressed_seq_t *cs, cmph_uint32 *vals_table,
  ...
  cs->store_table = (cmph_uint32 *)calloc(((cs->total_length + 31) >> 5),
                                          sizeof(cmph_uint32));
  cs->total_length = 0;

  for (i = 0; i < cs->n; i++) {
    if (vals_table[i] == 0) continue;
    stored_value = vals_table[i] - ((1U << lengths[i]) - 1U);
    set_bits_at_pos(cs->store_table, cs->total_length, stored_value,
                    lengths[i]);
    cs->total_length += lengths[i];
  };
  ...
  cs->length_rems = (cmph_uint32 *)calloc(BITS_TABLE_SIZE(cs->n, cs->rem_r),
                                          sizeof(cmph_uint32));
  ...
  for (i = 0; i < cs->n; i++) {
    cs->total_length += lengths[i];
    set_bits_value(cs->length_rems, i, cs->total_length & rems_mask, cs->rem_r,
                   rems_mask);
  ...
  // FABIANO: before it was (cs->total_length >> cs->rem_r) + 1. But I wiped out
  // the + 1 because
...
cmph_uint32 compressed_seq_get_space_usage(compressed_seq_t *cs) {
  ...
  space_usage +=
      ((cs->total_length + 31) >> 5) * (cmph_uint32)sizeof(cmph_uint32) * 8;
  space_usage +=
      BITS_TABLE_SIZE(cs->n, cs->rem_r) * (cmph_uint32)sizeof(cmph_uint32) * 8;
...
void compressed_seq_dump(compressed_seq_t *cs, char **buf,
                         cmph_uint32 *buflen) {
  ...
  *buflen = 4 * (cmph_uint32)sizeof(cmph_uint32) + sel_size + length_rems_size +
            store_table_size;
...
void compressed_seq_load(compressed_seq_t *cs, const char *buf,
  ...
  cs->length_rems =
      (cmph_uint32 *)calloc(length_rems_size, sizeof(cmph_uint32));
  length_rems_size *= 4;
  ...
  cs->store_table =
      (cmph_uint32 *)calloc(store_table_size, sizeof(cmph_uint32));
  store_table_size *= 4;
...
cmph_uint32 compressed_seq_packed_size(compressed_seq_t *cs) {
  ...
  register cmph_uint32 store_table_size =
      ((cs->total_length + 31) >> 5) * (cmph_uint32)sizeof(cmph_uint32);
  register cmph_uint32 length_rems_size =
      BITS_TABLE_SIZE(cs->n, cs->rem_r) * (cmph_uint32)sizeof(cmph_uint32);
  return 4 * (cmph_uint32)sizeof(cmph_uint32) + sel_size + store_table_size +
         length_rems_size;
}
