...
static inline void select_insert_0(cmph_uint32 *buffer) { (*buffer) >>= 1; };
...
cmph_uint32 select_get_space_usage(select_t *sel) {
  ...
  vec_size = (nbits + 31) >> 5;
  sel_table_size =
      (sel->n >> NBITS_STEP_SELECT_TABLE) +
      1;  // (sel->n >> NBITS_STEP_SELECT_TABLE) = (sel->n/STEP_SELECT_TABLE)
...
static inline void select_generate_sel_table(select_t *sel) {
  ...
  for (;;) {
    ...
    sel->select_table[sel_table_idx] =
        select_lookup_table[bits_table[vec_idx - 1]][one_idx - old_part_sum] +
        ((vec_idx - 1) << 3);  // ((vec_idx - 1) << 3) = ((vec_idx - 1) * 8)
    one_idx += STEP_SELECT_TABLE;
    sel_table_idx++;
  };
};

void select_generate(select_t *sel, cmph_uint32 *keys_vec, cmph_uint32 n,
                     cmph_uint32 m) {
  ...
  sel->select_table =
      (cmph_uint32 *)calloc(sel_table_size, sizeof(cmph_uint32));
...
static inline cmph_uint32 _select_query(cmph_uint8 *bits_table,
                                        cmph_uint32 *select_table,
                                        cmph_uint32 one_idx) {
  register cmph_uint32 vec_bit_idx, vec_byte_idx;
  register cmph_uint32 part_sum, old_part_sum;

  vec_bit_idx = select_table[one_idx >> NBITS_STEP_SELECT_TABLE];  // one_idx >>
  // NBITS_STEP_SELECT_TABLE =
  // one_idx/STEP_SELECT_TABLE
  vec_byte_idx = vec_bit_idx >> 3;  // vec_bit_idx / 8

  one_idx &= MASK_STEP_SELECT_TABLE;  // one_idx %= STEP_SELECT_TABLE == one_idx
...
static inline cmph_uint32 _select_next_query(cmph_uint8 *bits_table,
                                             cmph_uint32 vec_bit_idx) {
...
void select_dump(select_t *sel, char **buf, cmph_uint32 *buflen) {
  ...
  register cmph_uint32 vec_size =
      ((nbits + 31) >> 5) *
      (cmph_uint32)sizeof(cmph_uint32);  // (nbits + 31) >> 5 = (nbits + 31)/32
  register cmph_uint32 sel_table_size =
      ((sel->n >> NBITS_STEP_SELECT_TABLE) + 1) *
      (cmph_uint32)sizeof(cmph_uint32);  // (sel->n >> NBITS_STEP_SELECT_TABLE)
                                         // = (sel->n/STEP_SELECT_TABLE)
...
void select_load(select_t *sel, const char *buf,
                 cmph_uint32 buflen __attribute__((unused))) {
  register cmph_uint32 pos = 0;
  ...
  sel->select_table = (cmph_uint32 *)calloc(
      sel_table_size / sizeof(cmph_uint32), sizeof(cmph_uint32));
 ...
 *  \brief Support the ability to pack a select structure function into a
 * preallocated contiguous memory space pointed by sel_packed.
 *  \param sel points to the select structure
 *  \param sel_packed pointer to the contiguous memory area used to store the
 * select structure. The size of sel_packed must be at least @see
 * select_packed_size
 */
...
cmph_uint32 select_query_packed(void *sel_packed, cmph_uint32 one_idx) {
  ...
  register cmph_uint32 vec_size =
      (nbits + 31) >> 5;  // (nbits + 31) >> 5 = (nbits + 31)/32
...
cmph_uint32 select_next_query_packed(void *sel_packed,
                                     cmph_uint32 vec_bit_idx) {
