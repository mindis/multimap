...
struct _compressed_seq_t {
  ...
  // The length in bits of each value is decomposed into two compnents: the
  // lg(n) MSBs are stored in rank_select data structure
  // the remaining LSBs are stored in a table of n cells, each one of rem_r
  // bits.
  cmph_uint32 rem_r;
...
/** \fn void compressed_seq_generate(compressed_seq_t * cs, cmph_uint32 *
 * vals_table, cmph_uint32 n);
 ...
 */
void compressed_seq_generate(compressed_seq_t *cs, cmph_uint32 *vals_table,
                             cmph_uint32 n);

/** \fn cmph_uint32 compressed_seq_query(compressed_seq_t * cs, cmph_uint32
 * idx);
 *  \brief Returns the value stored at index @see idx of the compressed sequence
 * structure.
 *  \param cs points to the compressed sequence structure
 *  \param idx index to retrieve the value from
 *  \return the value stored at index @see idx of the compressed sequence
 * structure
 */
 ...
 *  \brief Support the ability to pack a compressed sequence structure into a
 * preallocated contiguous memory space pointed by cs_packed.
 *  \param cs points to the compressed sequence structure
 *  \param cs_packed pointer to the contiguous memory area used to store the
 * compressed sequence structure. The size of cs_packed must be at least @see
 * compressed_seq_packed_size
 */
void compressed_seq_pack(compressed_seq_t *cs, void *cs_packed);

/** \fn cmph_uint32 compressed_seq_packed_size(compressed_seq_t *cs);
 *  \brief Return the amount of space needed to pack a compressed sequence
 * structure.
 *  \return the size of the packed compressed sequence structure or zero for
 * failures
 */
cmph_uint32 compressed_seq_packed_size(compressed_seq_t *cs);

/** \fn cmph_uint32 compressed_seq_query_packed(void * cs_packed, cmph_uint32
 * idx);
 *  \brief Returns the value stored at index @see idx of the packed compressed
 * sequence structure.
 *  \param cs_packed is a pointer to a contiguous memory area
 *  \param idx is the index to retrieve the value from
 *  \return the value stored at index @see idx of the packed compressed sequence
 * structure
 */
