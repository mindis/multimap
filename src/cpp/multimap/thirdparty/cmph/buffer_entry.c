...
void buffer_entry_set_capacity(buffer_entry_t *buffer_entry,
...
static void buffer_entry_load(buffer_entry_t *buffer_entry) {
  ...
  buffer_entry->buff =
      (cmph_uint8 *)calloc((size_t)buffer_entry->capacity, sizeof(cmph_uint8));
  buffer_entry->nbytes =
      (cmph_uint32)fread(buffer_entry->buff, (size_t)1,
                         (size_t)buffer_entry->capacity, buffer_entry->fd);
...
cmph_uint8 *buffer_entry_read_key(buffer_entry_t *buffer_entry,
  ...
  if ((buffer_entry->pos + lacked_bytes) > buffer_entry->nbytes) {
    ...
    if (copied_bytes != 0)
      memcpy(keylen, buffer_entry->buff + buffer_entry->pos,
             (size_t)copied_bytes);
    buffer_entry_load(buffer_entry);
  }
  memcpy(keylen + copied_bytes, buffer_entry->buff + buffer_entry->pos,
         (size_t)lacked_bytes);
  ...
  if ((buffer_entry->pos + lacked_bytes) > buffer_entry->nbytes) {
    ...
    lacked_bytes = (buffer_entry->pos + lacked_bytes) - buffer_entry->nbytes;
    if (copied_bytes != 0) {
      memcpy(buf + sizeof(*keylen), buffer_entry->buff + buffer_entry->pos,
             (size_t)copied_bytes);
    }
    buffer_entry_load(buffer_entry);
  }
  memcpy(buf + sizeof(*keylen) + copied_bytes,
         buffer_entry->buff + buffer_entry->pos, (size_t)lacked_bytes);
  buffer_entry->pos += lacked_bytes;
  return buf;
}
