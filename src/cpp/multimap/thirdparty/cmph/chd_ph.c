...
static inline void chd_ph_bucket_clean(chd_ph_bucket_t *buckets,
...
chd_ph_bucket_t *chd_ph_bucket_new(cmph_uint32 nbuckets) {
  chd_ph_bucket_t *buckets =
      (chd_ph_bucket_t *)calloc(nbuckets, sizeof(chd_ph_bucket_t));
  return buckets;
}
...
void chd_ph_bucket_clean(chd_ph_bucket_t *buckets, cmph_uint32 nbuckets) {
  ...
  assert(buckets);
  for (i = 0; i < nbuckets; i++) buckets[i].size = 0;
}
static cmph_uint8 chd_ph_bucket_insert(chd_ph_bucket_t *buckets,
                                       chd_ph_map_item_t *map_items,
                                       chd_ph_item_t *items,
                                       cmph_uint32 nbuckets
                                       __attribute__((unused)),
                                       cmph_uint32 item_idx) {
  register cmph_uint32 i = 0;
...
void chd_ph_bucket_destroy(chd_ph_bucket_t *buckets) { free(buckets); }

static inline cmph_uint8 chd_ph_mapping(cmph_config_t *mph,
                                        chd_ph_bucket_t *buckets,
                                        chd_ph_item_t *items,
                                        cmph_uint32 *max_bucket_size);

static chd_ph_sorted_list_t *chd_ph_ordering(chd_ph_bucket_t **_buckets,
                                             chd_ph_item_t **items,
                                             cmph_uint32 nbuckets,
                                             cmph_uint32 nitems,
                                             cmph_uint32 max_bucket_size);

static cmph_uint8 chd_ph_searching(chd_ph_config_data_t *chd_ph,
                                   chd_ph_bucket_t *buckets,
                                   chd_ph_item_t *items,
                                   cmph_uint32 max_bucket_size,
...
static inline double chd_ph_get_entropy(cmph_uint32 *disp_table, cmph_uint32 n,
                                        cmph_uint32 max_probes) {
  register cmph_uint32 *probe_counts =
      (cmph_uint32 *)calloc(max_probes, sizeof(cmph_uint32));
  ...
  for (i = 0; i < max_probes; i++) {
    if (probe_counts[i] > 0)
      entropy -=
          probe_counts[i] * log((double)probe_counts[i] / (double)n) / log(2);
  };
  free(probe_counts);
  return entropy;
};
...
void chd_ph_config_set_keys_per_bin(cmph_config_t *mph,
  ...
  *max_bucket_size = 0;
  while (1) {
    ...
    for (i = 0; i < chd_ph->m; i++) {
      ...
      // 			if(buckets[g].size == (chd_ph->keys_per_bucket
      // <<
      // 2))
      // 			{
      // 				DEBUGP("BUCKET = %u -- SIZE = %u --
      // MAXIMUM
...
chd_ph_sorted_list_t *chd_ph_ordering(chd_ph_bucket_t **_buckets,
                                      chd_ph_item_t **_items,
                                      cmph_uint32 nbuckets, cmph_uint32 nitems,
                                      cmph_uint32 max_bucket_size) {
  chd_ph_sorted_list_t *sorted_lists = (chd_ph_sorted_list_t *)calloc(
      max_bucket_size + 1, sizeof(chd_ph_sorted_list_t));
  ...
  // Determine final position of list of buckets into the contiguous array that
  // will store all the buckets
  ...
  output_buckets =
      calloc(nbuckets,
             sizeof(chd_ph_bucket_t));  // everything is initialized with zero
  ...
  for (i = 0; i < nbuckets; i++) {
    ...
    position =
        sorted_lists[bucket_size].buckets_list + sorted_lists[bucket_size].size;
  ...
  // Return the buckets sorted in new order and free the old buckets sorted in
  // old order
  free(input_buckets);
  ...
  for (bucket_size = 1; bucket_size <= max_bucket_size; bucket_size++) {
    for (i = sorted_lists[bucket_size].buckets_list;
         i < sorted_lists[bucket_size].size +
  ...
  // Return the items sorted in new order and free the old items sorted in old
  // order
  free(input_items);
  (*_items) = output_items;
  return sorted_lists;
...
static inline cmph_uint8 place_bucket_probe(
  ...
  if (chd_ph->keys_per_bin > 1) {
    ...
    {
      position = (cmph_uint32)(
          (item->f + ((cmph_uint64)item->h) * probe0_num + probe1_num) %
          chd_ph->n);
...
static inline cmph_uint8 place_bucket(chd_ph_config_data_t *chd_ph,
                                      chd_ph_bucket_t *buckets,
                                      chd_ph_item_t *items,
                                      cmph_uint32 max_probes,
                                      cmph_uint32 *disp_table,
...

{
  ...
  while (1) {
    if (place_bucket_probe(chd_ph, buckets, items, probe0_num, probe1_num,
                           bucket_num, size)) {
      disp_table[buckets[bucket_num].bucket_id] =
...
static inline cmph_uint8 place_buckets1(chd_ph_config_data_t *chd_ph,
                                        chd_ph_bucket_t *buckets,
                                        chd_ph_item_t *items,
                                        cmph_uint32 max_bucket_size,
                                        chd_ph_sorted_list_t *sorted_lists,
  ...
  for (i = max_bucket_size; i > 0; i--) {
    ...
    while (curr_bucket < sorted_lists[i].size + sorted_lists[i].buckets_list) {
      if (!place_bucket(chd_ph, buckets, items, max_probes, disp_table,
                        curr_bucket, i)) {
        return 0;
      }
      curr_bucket++;
...
static inline cmph_uint8 place_buckets2(chd_ph_config_data_t *chd_ph,
                                        chd_ph_bucket_t *buckets,
                                        chd_ph_item_t *items,
                                        cmph_uint32 max_bucket_size,
                                        chd_ph_sorted_list_t *sorted_lists,
  ...
  for (i = max_bucket_size; i > 0; i--) {
    ...
    while (sorted_lists[i].size != 0) {
      ...
      for (j = 0, non_placed_bucket = 0; j < sorted_lists[i].size; j++) {
        ...
        if (place_bucket_probe(chd_ph, buckets, items, probe0_num, probe1_num,
                               curr_bucket, i)) {
          disp_table[buckets[curr_bucket].bucket_id] =
              probe0_num + probe1_num * chd_ph->n;
          // 					DEBUGP("BUCKET %u PLACED --- DISPLACEMENT =
          // %u\n",
...
// curr_bucket);
#ifdef DEBUG
          items_list = buckets[non_placed_bucket + sorted_lists[i].buckets_list]
                           .items_list;
          bucket_id = buckets[non_placed_bucket + sorted_lists[i].buckets_list]
                          .bucket_id;
#endif
          buckets[non_placed_bucket + sorted_lists[i].buckets_list].items_list =
              buckets[curr_bucket].items_list;
          buckets[non_placed_bucket + sorted_lists[i].buckets_list].bucket_id =
...
cmph_uint8 chd_ph_searching(chd_ph_config_data_t *chd_ph,
                            chd_ph_bucket_t *buckets, chd_ph_item_t *items,
                            cmph_uint32 max_bucket_size,
                            chd_ph_sorted_list_t *sorted_lists,
                            cmph_uint32 max_probes, cmph_uint32 *disp_table) {
  if (chd_ph->use_h) {
    return place_buckets2(chd_ph, buckets, items, max_bucket_size, sorted_lists,
                          max_probes, disp_table);
  } else {
    return place_buckets1(chd_ph, buckets, items, max_bucket_size, sorted_lists,
                          max_probes, disp_table);
  }
}
...
static inline cmph_uint8 chd_ph_check_bin_hashing(
  ...
  if (chd_ph->keys_per_bin > 1)
    ...
    memset(chd_ph->occup_table, 0,
           ((chd_ph->n + 31) / 32) * sizeof(cmph_uint32));
...
cmph_t *chd_ph_new(cmph_config_t *mph, double c) {
  ...
  chd_ph->n =
      (cmph_uint32)(chd_ph->m / (chd_ph->keys_per_bin * load_factor)) + 1;

  // Round the number of bins to the prime immediately above
  if (chd_ph->n % 2 == 0) chd_ph->n++;
  for (;;) {
    if (check_primality(chd_ph->n) == 1) break;
  ...
  if (chd_ph->keys_per_bin == 1)
    chd_ph->occup_table =
        (cmph_uint8 *)calloc(((chd_ph->n + 31) / 32), sizeof(cmph_uint32));
  else
  ...
  while (1) {
    ...
    if (mph->verbosity) {
      fprintf(
          stderr,
          "Starting mapping step for mph creation of %u keys with %u bins\n",
          chd_ph->m, chd_ph->n);
    }
    ...
    sorted_lists = chd_ph_ordering(&buckets, &items, chd_ph->nbuckets,
                                   chd_ph->m, max_bucket_size);
    ...
    searching_success =
        chd_ph_searching(chd_ph, buckets, items, max_bucket_size, sorted_lists,
                         max_probes, disp_table);
  ...
  register double entropy =
      chd_ph_get_entropy(disp_table, chd_ph->nbuckets, max_probes);
  DEBUGP("Entropy = %.4f\n", entropy / chd_ph->m);
#endif
...
cleanup:
  ...
  fprintf(stdout, "%u\t%.2f\t%u\t%.4f\t%.4f\t%.4f\t%.4f\n", chd_ph->m,
          ...
          space_usage / (double)chd_ph->m, space_lower_bound,
          entropy / chd_ph->m);
#endif
...
cmph_uint32 chd_ph_search(cmph_t *mphf, const char *key, cmph_uint32 keylen) {
  ...
  position = (cmph_uint32)((f + ((cmph_uint64)h) * probe0_num + probe1_num) %
                           chd_ph->n);
  return position;
}
...
cmph_uint32 chd_ph_packed_size(cmph_t *mphf) {
  ...
  return (cmph_uint32)(sizeof(CMPH_ALGO) + hash_state_pack_size + cs_pack_size +
                       3 * sizeof(cmph_uint32));
}

cmph_uint32 chd_ph_search_packed(void *packed_mphf, const char *key,
                                 cmph_uint32 keylen) {
  ...
  register cmph_uint32 *ptr =
      (cmph_uint32 *)(hl_ptr + hash_state_packed_size(hl_type));
  ...
  probe1_num = disp / n;
  position =
      (cmph_uint32)((f + ((cmph_uint64)h) * probe0_num + probe1_num) % n);
  return position;
}
