...
const char *cmph_names[] = {"bmz", "bmz8",   "chm",    "brz", "fch",
                            "bdz", "bdz_ph", "chd_ph", "chd", NULL};
...
static cmph_io_adapter_t *cmph_io_struct_vector_new(void *vector,
...
static int key_struct_vector_read(void *data, char **key, cmph_uint32 *keylen) {
  ...
  *keylen = cmph_struct_vector->key_len;
  size = *keylen;
  *key = (char *)malloc(size);
  memcpy(*key, (keys_vd + (cmph_struct_vector->position *
                           cmph_struct_vector->struct_size) +
                cmph_struct_vector->key_offset),
         size);
...
static void key_nlfile_dispose(void *data __attribute__((unused)), char *key,
                               cmph_uint32 keylen __attribute__((unused))) {
  free(key);
}

static void key_vector_dispose(void *data __attribute__((unused)), char *key,
                               cmph_uint32 keylen __attribute__((unused))) {
  free(key);
}
...
static cmph_io_adapter_t *cmph_io_struct_vector_new(void *vector,
  ...
  cmph_struct_vector_t *cmph_struct_vector =
      (cmph_struct_vector_t *)malloc(sizeof(cmph_struct_vector_t));
  assert(key_source);
...
static void cmph_io_struct_vector_destroy(cmph_io_adapter_t *key_source) {
  cmph_struct_vector_t *cmph_struct_vector =
      (cmph_struct_vector_t *)key_source->data;
...
cmph_io_adapter_t *cmph_io_struct_vector_adapter(void *vector,
                                                 ...
                                                 cmph_uint32 key_len,
                                                 cmph_uint32 nkeys) {
  cmph_io_adapter_t *key_source = cmph_io_struct_vector_new(
      vector, struct_size, key_offset, key_len, nkeys);
...
cmph_uint32 cmph_size(cmph_t *mphf) { return mphf->size; }
...
/** cmph_uint32 cmph_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 cmph_search_packed(void *packed_mphf, const char *key,
                               cmph_uint32 keylen) {
  cmph_uint32 *ptr = (cmph_uint32 *)packed_mphf;
