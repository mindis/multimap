...
/* static const cmph_uint8 bitmask[8] = { 1, 1 << 1, 1 << 2, 1 << 3, 1 << 4, 1
 * << 5, 1 << 6, 1 << 7 }; */
  ...
  for (i = 0; i < g->nnodes; ++i) {
    ...
    if (e != EMPTY) {
      ...
      while ((e = g->next[e]) != EMPTY) {
        printf("%u -> %u\n", g->edges[abs_edge(e, 0)],
               g->edges[abs_edge(e, 1)]);
      }
    }
  }
  return;
...
static int check_edge(graph_t *g, cmph_uint32 e, cmph_uint32 v1,
                      cmph_uint32 v2) {
  DEBUGP("Checking edge %u %u looking for %u %u\n", g->edges[abs_edge(e, 0)],
         g->edges[abs_edge(e, 1)], v1, v2);
...
static cmph_uint8 find_degree1_edge(graph_t *g, cmph_uint32 v,
                                    cmph_uint8 *deleted, cmph_uint32 *e) {
...
int graph_is_cyclic(graph_t *g) {
  ...
  cmph_uint32 v;
  cmph_uint8 *deleted =
      (cmph_uint8 *)malloc((g->nedges * sizeof(cmph_uint8)) / 8 + 1);
...
cmph_uint8 graph_node_is_critical(graph_t *g,
                                  cmph_uint32 v) /* included -- Fabiano */
{
...
void graph_obtain_critical_nodes(graph_t *g) /* included -- Fabiano*/
{
  ...
  g->critical_nodes =
      (cmph_uint8 *)malloc((g->nnodes * sizeof(cmph_uint8)) / 8 + 1);
  g->ncritical_nodes = 0;
...
cmph_uint8 graph_contains_edge(graph_t *g, cmph_uint32 v1,
                               cmph_uint32 v2) /* included -- Fabiano*/
{
  cmph_uint32 e;
  e = g->first[v1];
  if (e == EMPTY) return 0;
...
cmph_uint32 graph_vertex_id(graph_t *g, cmph_uint32 e,
                            cmph_uint32 id) /* included -- Fabiano*/
{
