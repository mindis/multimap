...
static char *brz_copy_partial_fch_mphf(brz_config_data_t *brz, fch_data_t *fchf,
                                       cmph_uint32 index, cmph_uint32 *buflen);
static char *brz_copy_partial_bmz8_mphf(brz_config_data_t *brz,
                                        bmz8_data_t *bmzf, cmph_uint32 index,
                                        cmph_uint32 *buflen);
...
void brz_config_set_memory_availability(cmph_config_t *mph,
...
cmph_t *brz_new(cmph_config_t *mph, double c) {
  ...
  while (1) {
    ...
    if (!ok) {
      ...
      brz->h0 = NULL;
      DEBUGP(
          "%u iterations remaining to create the graphs in a external file\n",
          iterations);
      if (mph->verbosity) {
        fprintf(stderr,
                "Failure: A graph with more than 255 keys was created - %u "
  ...
  if (iterations == 0) {
    DEBUGP(
        "Graphs with more than 255 keys were created in all 20 iterations\n");
    free(brz->size);
    return NULL;
  }
...
static int brz_gen_mphf(cmph_config_t *mph) {
  ...
  cmph_uint32 *buckets_size =
      (cmph_uint32 *)calloc((size_t)brz->k, sizeof(cmph_uint32));
  ...
  for (e = 0; e < brz->m; ++e) {
    ...
    /* Buffers management */
    if (memory_usage + keylen + sizeof(keylen) >
        brz->memory_availability)  // flush buffers
    {
      if (mph->verbosity) {
      ...
      memory_usage = 0;
      keys_index =
          (cmph_uint32 *)calloc((size_t)nkeys_in_buffer, sizeof(cmph_uint32));
      for (i = 0; i < nkeys_in_buffer; i++) {
        memcpy(&keylen1, buffer + memory_usage, sizeof(keylen1));
        h0 = hash(brz->h0, (char *)(buffer + memory_usage + sizeof(keylen1)),
                  keylen1) %
             brz->k;
      ...
      filename =
          (char *)calloc(strlen((char *)(brz->tmp_dir)) + 11, sizeof(char));
      ...
      for (i = 0; i < nkeys_in_buffer; i++) {
        ...
        nbytes = fwrite(buffer + keys_index[i], (size_t)1,
                        keylen1 + sizeof(keylen1), tmp_fd);
      }
      nkeys_in_buffer = 0;
      memory_usage = 0;
  ...
  nbytes =
      fwrite(cmph_names[CMPH_BRZ], (size_t)(strlen(cmph_names[CMPH_BRZ]) + 1),
             (size_t)1, brz->mphf_fd);
  ...
  nbytes = fwrite(&(brz->k), sizeof(cmph_uint32), (size_t)1,
                  brz->mphf_fd);  // number of MPHFs
  nbytes =
  ...
  e = 0;
  keys_vd =
      (cmph_uint8 **)calloc((size_t)MAX_BUCKET_SIZE, sizeof(cmph_uint8 *));
  nkeys_vd = 0;
  error = 0;
  while (e < brz->m) {
    ...
    if (nkeys_vd == brz->size[cur_bucket])  // Generating mphf for each bucket.
    {
      ...
      if (mphf_tmp == NULL) {
        if (mph->verbosity)
          fprintf(stderr,
                  "ERROR: Can't generate MPHF for bucket %u out of %u\n",
                  cur_bucket + 1, brz->k);
        error = 1;
      ...
      switch (brz->algo) {
        case CMPH_FCH: {
          ...
          bufmphf =
              brz_copy_partial_fch_mphf(brz, fchf, cur_bucket, &buflenmphf);
        } break;
        case CMPH_BMZ8: {
          bmz8_data_t *bmzf = NULL;
          bmzf = (bmz8_data_t *)mphf_tmp->data;
          bufmphf =
              brz_copy_partial_bmz8_mphf(brz, bmzf, cur_bucket, &buflenmphf);
        } break;
        default:
          assert(0);
      }
...
static char *brz_copy_partial_fch_mphf(brz_config_data_t *brz
                                       __attribute__((unused)),
                                       fch_data_t *fchf, cmph_uint32 index
                                       __attribute__((unused)),
                                       cmph_uint32 *buflen) {
  cmph_uint32 i = 0;
  ...
  *buflen = buflenh1 + buflenh2 + n + 2U * (cmph_uint32)sizeof(cmph_uint32);
  ...
  for (i = 0; i < n; i++)
    memcpy(buf + 2 * sizeof(cmph_uint32) + buflenh1 + buflenh2 + i,
           (fchf->g + i), (size_t)1);
  free(bufh1);
  free(bufh2);
  return buf;
...
static char *brz_copy_partial_bmz8_mphf(brz_config_data_t *brz,
  ...
  *buflen = buflenh1 + buflenh2 + n + 2U * (cmph_uint32)sizeof(cmph_uint32);
  ...
  memcpy(buf + sizeof(cmph_uint32) + buflenh1, &buflenh2, sizeof(cmph_uint32));
  memcpy(buf + 2 * sizeof(cmph_uint32) + buflenh1, bufh2, (size_t)buflenh2);
  memcpy(buf + 2 * sizeof(cmph_uint32) + buflenh1 + buflenh2, bmzf->g,
         (size_t)n);
  free(bufh1);
  free(bufh2);
  return buf;
...
int brz_dump(cmph_t *mphf, FILE *fd) {
  ...
  register size_t nbytes __attribute__((unused));
  DEBUGP("Dumping brzf\n");
  // The initial part of the MPHF have already been dumped to disk during
  // construction
  // Dumping h0
...
static cmph_uint32 brz_bmz8_search(brz_data_t *brz, const char *key,
                                   cmph_uint32 keylen,
...
static cmph_uint32 brz_fch_search(brz_data_t *brz, const char *key,
                                  cmph_uint32 keylen,
                                  cmph_uint32 *fingerprint) {
  register cmph_uint32 h0;
...
cmph_uint32 brz_packed_size(cmph_t *mphf) {
  ...
  CMPH_HASH h1_type = hash_get_type(data->h1[0]);
  CMPH_HASH h2_type = hash_get_type(data->h2[0]);
  size = (cmph_uint32)(2 * sizeof(CMPH_ALGO) + 3 * sizeof(CMPH_HASH) +
                       hash_state_packed_size(h0_type) + sizeof(cmph_uint32) +
                       sizeof(double) + sizeof(cmph_uint8) * data->k +
                       sizeof(cmph_uint32) * data->k);
// pointers to g_is
#if defined(__ia64) || defined(__x86_64__)
...
static cmph_uint32 brz_bmz8_search_packed(cmph_uint32 *packed_mphf,
                                          const char *key, cmph_uint32 keylen,
                                          cmph_uint32 *fingerprint) {
  register CMPH_HASH h0_type = *packed_mphf++;
  register cmph_uint32 *h0_ptr = packed_mphf;
  packed_mphf = (cmph_uint32 *)(((cmph_uint8 *)packed_mphf) +
                                hash_state_packed_size(h0_type));
...
static cmph_uint32 brz_fch_search_packed(cmph_uint32 *packed_mphf,
                                         const char *key, cmph_uint32 keylen,
                                         cmph_uint32 *fingerprint) {
  register CMPH_HASH h0_type = *packed_mphf++;
...
/** cmph_uint32 brz_search(void *packed_mphf, const char *key, cmph_uint32
 * keylen);
 ...
 */
cmph_uint32 brz_search_packed(void *packed_mphf, const char *key,
                              cmph_uint32 keylen) {
