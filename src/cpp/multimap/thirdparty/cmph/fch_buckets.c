...
static void fch_bucket_reserve(fch_bucket_t *bucket, cmph_uint32 size) {
  ...
  if (bucket->capacity < size) {
    ...
    bucket->entries = (fch_bucket_entry_t *)realloc(
        bucket->entries, sizeof(fch_bucket_entry_t) * new_capacity);
...
static void fch_bucket_insert(fch_bucket_t *bucket, char *val,
                              cmph_uint32 val_length) {
  assert(bucket);
...
static cmph_uint32 fch_bucket_get_length(fch_bucket_t *bucket,
...
fch_buckets_t *fch_buckets_new(cmph_uint32 nbuckets) {
  ...
  assert(buckets);
  buckets->values =
      (fch_bucket_t *)calloc((size_t)nbuckets, sizeof(fch_bucket_t));
...
void fch_buckets_insert(fch_buckets_t *buckets, cmph_uint32 index, char *key,
                        cmph_uint32 length) {
  assert(index < buckets->nbuckets);
...
char *fch_buckets_get_key(fch_buckets_t *buckets, cmph_uint32 index,
                          cmph_uint32 index_key) {
  assert(index < buckets->nbuckets);
...
cmph_uint32 *fch_buckets_get_indexes_sorted_by_size(fch_buckets_t *buckets) {
  ...
  cmph_uint32 *nbuckets_size =
      (cmph_uint32 *)calloc((size_t)buckets->max_size + 1, sizeof(cmph_uint32));
  cmph_uint32 *sorted_indexes =
      (cmph_uint32 *)calloc((size_t)buckets->nbuckets, sizeof(cmph_uint32));

  // collect how many buckets for each size.
  for (cmph_uint32 i = 0; i < buckets->nbuckets; i++)
  ...
  for (int i = (int)buckets->max_size - 1; i >= 0; i--) {
    sum += value;
    value = nbuckets_size[i];
    nbuckets_size[i] = sum;
  }
  for (cmph_uint32 i = 0; i < buckets->nbuckets; i++) {
    sorted_indexes[nbuckets_size[fch_bucket_size(buckets->values + i)]] =
        (cmph_uint32)i;
    nbuckets_size[fch_bucket_size(buckets->values + i)]++;
  }
  free(nbuckets_size);
